...
function formatExplanationLine(text: string) {
                  ...
                  <div className="mobile-obsidian-callout" aria-label="Açıklamalı çözüm">
                    ...
                    <div className="callout-content">
                      {q.explanation.split("\n").map((expLine, eIdx) => {
                        ...
                        if (!trimmed) {
                          return <div key={eIdx} className="callout-spacer" />;
                        }
                        if (trimmed.startsWith("✅ DOĞRU CEVAP") || trimmed.startsWith("✅")) {
                          return (
                            <div key={eIdx} className="callout-correct-banner">
                              {formatExplanationLine(expLine)}
                            </div>
                          );
                        }
                        if (trimmed.startsWith("🔍 ÇELDİRİCİLERİN ANALİZİ") || trimmed.startsWith("🔍")) {
                          return (
                            <div key={eIdx} className="callout-analysis-header">
                              {formatExplanationLine(expLine)}
                            </div>
                          );
                        }
                        if (trimmed.startsWith("💡 KPSS HAFIZA KARTI") || trimmed.startsWith("💡")) {
                          return (
                            <div key={eIdx} className="callout-tip-banner">
                              {formatExplanationLine(expLine)}
                            </div>
                          );
                        }
                        if (trimmed.startsWith("•") || trimmed.startsWith("-")) {
                          return (
                            <div key={eIdx} className="callout-distractor-item">
                              {formatExplanationLine(expLine)}
                            </div>
                          );
                        }
                        return <p key={eIdx}>{formatExplanationLine(expLine)}</p>;
                      })}
                    </div>
